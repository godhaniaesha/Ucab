...
async function findNearbyDrivers(coordinates, vehicleType='standard', radiusMeters=5000, preferredVehicleModel=null, preferredVehicleId=null){
  try {
    ...
    if(preferredVehicleModel){
      ...
      const vehicles = await Vehicle.find({ $or: [ { model: { $regex: preferredVehicleModel, $options: 'i' } }, { make: { $regex: preferredVehicleModel, $options: 'i' } } ] }).limit(50);
      if(vehicles && vehicles.length){
        const vehicleIds = vehicles.map(v=>v._id);
        const drivers = await User.find({ ...baseQuery, vehicle: { $in: vehicleIds } }).populate('vehicle').limit(20);
        if(drivers && drivers.length) return drivers;
      }
    }
    ...
    const drivers = await User.find({ ...baseQuery }).populate('vehicle').limit(50);
    return drivers;
  } catch(err){
...
function waitForDriverResponse(bookingId, driverId, timeout=30000){
  return new Promise((resolve) => {
    ...
    let resolved=false;
    const handler = (accepted) => { if(!resolved){ resolved=true; ioInstance.off(eventName, handler); resolve(accepted); } };
    ioInstance.once(eventName, handler);
    setTimeout(()=>{ if(!resolved){ resolved=true; ioInstance.off(eventName, handler); resolve(false); } }, timeout);
  });
}
...
function initSockets(io){
  ...
  io.on('connection', (socket) => {
    socket.on('register_driver', (data) => { if(data && data.driverId){ driverSockets.set(data.driverId, socket); socket.driverId = data.driverId; } });
    socket.on('register_passenger', (data) => { if(data && data.passengerId){ passengerSockets.set(data.passengerId, socket); socket.passengerId = data.passengerId; } });

    socket.on('accept_booking', async (data) => {
      try {
        const { bookingId, driverId } = data;
        const booking = await Booking.findById(bookingId);
        if(!booking || booking.status !== BOOKING_STATUS.ASSIGNED){ socket.emit('booking_error', { message:'Not available' }); return; }
        booking.status = BOOKING_STATUS.ACCEPTED; booking.acceptedAt = new Date(); await booking.save();
        const passengerSocket = passengerSockets.get(booking.passenger.toString());
        if(passengerSocket) passengerSocket.emit('booking_accepted', { bookingId: booking._id, driver: await User.findById(driverId).populate('vehicle') });
        ioInstance.emit(`driver_response_${bookingId}_${driverId}`, true);
      } catch(err){ console.error(err); socket.emit('booking_error', { message:'Error' }); }
    });

    socket.on('reject_booking', async (data) => { const { bookingId, driverId } = data; await User.findByIdAndUpdate(driverId, { status: USER_STATUS.AVAILABLE }); ioInstance.emit(`driver_response_${bookingId}_${driverId}`, false); });

    socket.on('driver_location', async (data) => {
      try {
        const { driverId, coordinates, currentBookingId } = data;
        await User.findByIdAndUpdate(driverId, { location: { type:'Point', coordinates }, lastActiveAt: new Date() });
        if(currentBookingId){
          await Booking.findByIdAndUpdate(currentBookingId, { driverLocation: { type:'Point', coordinates } });
          const booking = await Booking.findById(currentBookingId);
          if(booking){
            const passengerSocket = passengerSockets.get(booking.passenger.toString());
            if(passengerSocket) passengerSocket.emit('driver_location_update', { bookingId: currentBookingId, location: coordinates });
          }
        }
