...
export default function SuperAdminTab() {
  ...
  const [activeTab, setActiveTab] = useState(
    localStorage.getItem("superAdminActiveTab") || "dashboard"
  );

  const tabs = [
    { id: "dashboard", name: "Dashboard", iconClass: "bi bi-house-door-fill", component: <SA_DashboardContent /> },
    { id: "manage-drivers", name: "Manage Drivers", iconClass: "bi bi-person-badge-fill", component: <SA_ManageDriversContent /> },
    { id: "manage-passengers", name: "Manage Passengers", iconClass: "bi bi-people-fill", component: <SA_ManagePassengersContent /> },
              ...
              {tabs.map((tab) => (
                <button
                  ...
                  className={`d_nav_tab btn text-start py-3 px-4 rounded-3 text-nowrap d-flex align-items-center ${
                    activeTab === tab.id ? "active bg-white text-success shadow" : "text-light"
                  }`}
              ...
              <div className="d_nav_tabs_container d-flex gap-2">
                {tabs.map((tab) => (
                  ...
                  >
                    ...
                    <span className="tab-text fw-semibold" style={{ fontSize: "0.7rem" }}>{tab.name}</span>
                  </button>
                ))}
              </div>
            </div>
